...
namespace mydak::args {
    ...
    #pragma region Setup
    constexpr auto max_int8_t = std::numeric_limits<std::int8_t>::max();

    static constexpr auto tuple_boy = make_parameters(
        make_parameter<"--connect-tries", details::TYPE_SMALL_NUMBER>(1, max_int8_t, 3),
        make_parameter<"--wait-time", details::TYPE_SMALL_NUMBER>(0, max_int8_t, 2),
        make_parameter<"--wait-time-add", details::TYPE_SMALL_NUMBER>(1, max_int8_t, 2),
        make_parameter<"--public-key", details::TYPE_STRING>(64, 64, ""),
        make_parameter<"--recipient", details::TYPE_STRING>(64, 64, "")
    );

    using parameters_count = std::integral_constant<std::size_t,  std::tuple_size_v<std::decay_t<decltype(std::get<0>(tuple_boy))>>>;
    ...
    static constexpr tools::static_map<parameters_count::value, std::size_t> options_indices = std::apply([](auto&&... args) {
    ...
    struct parameters_accessor {
        ...
        constexpr parameters_accessor() = default;
        explicit constexpr parameters_accessor(std::array<parameter_variants, parameters_count::value> parameters) : parameters(parameters) {}

        template <std::size_t N>
        requires (N < parameters_count::value)
        auto get() const {
    ...
    private:
        std::array<parameter_variants, parameters_count::value> parameters;
    };
