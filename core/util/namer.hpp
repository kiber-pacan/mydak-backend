...
namespace mydak::namer {
    namespace detail {
        ...
        inline std::array<std::string_view, 256> icons = {
            "😀", "😃", "😄", "😁", "😆", "😅", "🤣", "😂",
            "🙂", "🙃", "😉", "😊", "😇", "🥰", "😍", "🤩",
            "😘", "😗", "😚", "😙", "😋", "😛", "😜", "🤪",
            "😝", "🤑", "🤗", "🤭", "🤫", "🤔", "🤐", "🤨",
    ...
    inline std::string_view get_icon(std::uint16_t value) {
        constexpr std::size_t max_value = std::numeric_limits<std::decay_t<decltype(value)>>::max() + 1;
        const auto index = value * std::size(detail::icons) / max_value;
        return detail::icons[index];
    }
}
