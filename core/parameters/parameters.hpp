...
namespace mydak::args {
    namespace details {
        static constexpr std::size_t TYPE_UINT8 = 0;
        static constexpr std::size_t TYPE_STRING = 1;
        static constexpr std::size_t TYPE_IP = 2;
        static constexpr std::size_t TYPE_UINT16 = 3;
    }
    ...
    struct raw_parameter {
        ...
        std::integral_constant<std::size_t, Index> type_index; // std::variant index
        tools::static_string<Option_String_Size> option; // Option name
        std::tuple<Args...> args; // Arguments for parameter initialization
    };
    // Deduction guide
    ...
    template <std::size_t Type_Index, typename... Args, std::size_t... Indices>
    constexpr auto make_from_indices(
        const std::tuple<Args...>& tuple,
        std::index_sequence<Indices...>
    ) {
        return parameter<Type_Index>(std::get<Indices>(tuple)...);
    }

    template <std::size_t Type_Index, typename... Args>
    constexpr auto make_from_tuple(
        const std::tuple<Args...>& tuple
    ) {
        return make_from_indices<Type_Index>(tuple, std::make_index_sequence<sizeof...(Args)>());
    }
     ...
     *
     *
     * @param raw_parameters mydak::args::raw_parameter
     *
     ...
     *       @endcode
     */
    template <typename... Raw_Parameter>
    constexpr auto make_parameters(Raw_Parameter&&... raw_parameters)
    requires (!std::is_lvalue_reference_v<Raw_Parameter> && ...) // All pairs should be rvalues
    {
        static_assert((is_valid_raw_parameter<Raw_Parameter>::value && ...),
            "Provided type is not mydak::args::raw_parameter!");
        ...
        // Make array of paramameter variantsstd::pair<std::integral_constant<std::size_t, N>, std::tuple<Args...>>
        auto parameters = std::array<parameter_variants, sizeof...(raw_parameters)>{
           parameter_variants(make_from_tuple<raw_parameters.type_index>(raw_parameters.args))...
        };

        // Make tuple of option names
        auto option_names = std::make_tuple(raw_parameters.option...);

        // returning new array and empty type_indices object
        return std::make_tuple(parameters, option_names, type_index);
    }
    #pragma endregion

    #pragma region Setup
    static constexpr auto tuple_boy = make_parameters(
        make_parameter<"--connect_tries", details::TYPE_UINT8>(1, 64, 3),
        make_parameter<"--wait_time", details::TYPE_UINT8>(1, 64, 3),
        make_parameter<"--wait_time_add", details::TYPE_UINT8>(1, 64, 3),
        ...
        make_parameter<"--password", details::TYPE_STRING>(3, 256, "password"),
        make_parameter<"--ip", details::TYPE_IP>("127.0.0.1"),
        make_parameter<"--port", details::TYPE_UINT16>(0, std::numeric_limits<std::uint16_t>::max(), 8888)
    );
